...
Adapted from MiniAuth: https://github.com/farzadghanei/miniauth/tree/master
"""
...
class AbstractStorage(object):
    ...
    @abstractmethod
    def get_record(self, user:int) -> Any:
        ...
        modl = self.user_model()
        return Repository(ssnmaker, modl).get_by_id(user)
...
#         """
#         db_con = sqlite3.connect(self.user_model)
...
#         logger.info('DB schema updated to version {} on "{}"'.format(self.user_model, version))
...
#             logger.info('No DB detected on "{}". Creating latest DB schema ...'.format(self.user_model))
